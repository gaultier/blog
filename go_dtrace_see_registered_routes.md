...
And for an API where that registration order *did* matter, we could also store an incrementing number in the map (globals are thread-safe in DTrace and can be safely mutated concurrently) to remember and print the order.
