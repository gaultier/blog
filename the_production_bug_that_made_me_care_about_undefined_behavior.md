...
1. If `T` is a [POD](https://en.cppreference.com/w/cpp/named_req/PODType) (Plain Old Data, pre C++11. The wording in the standard changed with C++11 but the idea remains), e.g. `Foo foo;` no initialization is performed at all. This is akin to doing `int a;` and then using `a`. This is obvious undefined behavior.
