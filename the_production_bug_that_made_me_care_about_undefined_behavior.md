...
This format is not ideal but that's what the software did. Obviously, either `error` or `succeeded` is set but not both or either (it's a XOR).
...
void handle() {
  ...
  try {
    ...
    response.data = "some stuff from the database";
  } catch(...) {
    response.error = true;
  }
  response.write();
...
But right after, I stumble upon official C++ examples that use this syntax. So now I am confused. C++ intialization rules are different from C, after all.
...
1. If `T` is a non class, non array type such as `int`, no initialization is performed at all. This is obvious undefined behavior.
1. If `T` is an array, this is fine: each element is default-initialized
1. If `T` is a [POD](https://en.cppreference.com/w/cpp/named_req/PODType) (Plain Old Data, pre C++11. The wording in the standard changed with C++11 but the idea remains), no initialization is performed at all. This is akin to doing `int a;` and then using `a`. This is obvious undefined behavior.
1. If `T` is not a POD, the default constructor is called, and is responsible for initializing all fields. It is easy to miss one, or even forget to implement a default constructor entirely, leading to undefined behavior.
...
The compiler (`clang`) does not catch this issue even with all warnings enabled. This is frustrating because the compiler happily generates, and calls, a default constructor that does not initialize all the fields. So, the caller is expected to set all the uninitialized fields to some value manually? This is non-sense to me.
...
I also wrote in my notes that `cppcheck` 'spots this without issues', but when I try it today, it does not spot anything even with `--enable=all`. So, maybe there is a regression there, or I am not using it correctly.


## The aftermath

I wrote a `libclang` plugin at the time to catch other instances of this problem in the codebase: https://github.com/gaultier/c/tree/master/libclang-plugin . 

Amazingly, there was only one other case, and it was a false positive because by chance, the caller set the uninitialized fields right after, like this:
...
- Syntax that looks like C but *sometimes* does something completely different than C, invisibly. This syntax can be perfectly correct (e.g. in the case of an array) or be undefined behavior. This makes code review really difficult.
- The compiler does not warn about undefined behavior and we have to rely on third-party linters, and these linters have limitations
...
But I am thankful for this bug, because it made me aware for the first time that undefined behavior is real and dangerous, for one simple reason: it makes your program behave completely differently than the code. By reading the code, you cannot predict the behavior of the program in any way. The code stopped to be the source of truth. And you can very easily, and invisibly, trigger undefined behavior.

And that is really, really bad when writing production code for mission critical systems.
