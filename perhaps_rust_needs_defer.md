...
*Discussions: [/r/rust](https://old.reddit.com/r/rust/comments/1gktuw6/perhaps_rust_needs_defer/?), [/r/programming](https://old.reddit.com/r/programming/comments/1gktum4/perhaps_rust_needs_defer/?), [HN](https://news.ycombinator.com/item?id=42058091), [lobsters](https://lobste.rs/s/2ka0ps/perhaps_rust_needs_defer)*
...
In this case, `MYLIB_free_foos` actually can receive an argument with a null pointer (the `data` field), which would then trigger an assert inside `Vec::from_raw_parts`. So we should check that in `MY_LIB_free_foos`:
