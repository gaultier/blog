...
`cargo run` is perfectly content with the code above. But the Rust compiler can and will silently assume that there is only one mutable pointer to `x`, and make optimizations, and generate machine code, based on that assumption, that this code breaks.
