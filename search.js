...
for (let pos of res) {
  ...
  const excerpt_len_around = 10;
  const excerpt = doc.text.slice(idx - excerpt_len_around, idx + needle.length + excerpt_len_around);
  console.log("[D004]", pos, title_i, doc.name, title, link, excerpt, idx - excerpt_len_around, idx + needle.length + excerpt_len_around);
}
