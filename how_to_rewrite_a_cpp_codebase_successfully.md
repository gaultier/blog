...
- Pick a prefix for all struct and functions in the C API exposed by the Rust code, even if it's just `RUST_xxx`, so that they are immediately identifiable and greppable. Just like `libcurl` has the prefix `curl_xxx`.
