...
Well, what if we wrote a whole program in assembly that opens a GUI window? It will be the hello world of the GUI world, but that still counts. Here is what we are working toward:

![Result](x11_x64_final.png)

I wanted to expand my knowledge of assembly and by doing something fun and motivating. It all originated from the observation that so many program binaries today are very big, often over 30 Mib (!), and I asked myself: How small a binary is needed to write a (very simplistic) GUI? Well, it turns out, very little *(spoiler alert: around 1 KiB!)*. 


*I am by no means an expert in assembly or in X11. I just hope to provide an entertaining, approachable article, something a beginner can understand. Something I wished I had found when I was learning those topics.*
...
No need for additional tooling to cross-compile, issues with dynamic libraries, etc. Just compile on Linux by defining the right variable on the command line, send the binary to your friend on FreeBSD, and it just works(tm). That's refreshing.
...
Where the server lives is actually not relevant for a client, it might run on the same machine or in datacenter far far away. Of course, in the context of a desktop computer in 2023, it will be running on the same machine, but that's a detail.
...
First, we tell nasm we are writing a 64 bit program and that we target x86_64. Then, we need a main function, which we call `_start` and needs to be visible since this is the entrypoint of our program (hence the `global` keyword):
...
```sh
$ file ./main
main: ELF 64-bit LSB executable, x86-64, version 1 (SYSV), no program header, not stripped
```

Using `ls -l`, we notice our executable is only `496` bytes big. It will grow a bit later, but not by much. 

Those bytes are for now just the ELF headers, which we can see with `readelf -l ./main`, and it tells us 
...
Following the System V ABI, which is required on Linux and other Unices, invoking a system call requires us to put the system call code in the register `rax`, the parameters to the syscall (up to 6) in the registers `rdi`, `rsi`, `rdx`, `rcx`, `r8`, `r9`, and additional parameters, if any, on the stack (which will not happen in this program so we can forget about it).
We then use the instruction `syscall` and check `rax` for the return value, `0` usually meaning: no error.

*Note that Linux has a 'fun' difference, which is that the fourth parameter is actually passed using the register `r10`.*
...
Another way to observe system calls made by a program is with `strace`, which will also prove very useful when troubleshooting:
...
We need to reserve (at least) 5 bytes on the stack, since that's the length in bytes of `hello`. We then pass the address on the stack of the beginning of the string to the `write` syscall, as well as its length:
...
**Note**: There is an optimization method that uses `rbp` as a standard register, which means we lose the information about the call stack. My advice is: never do this, it is no worth it.
...
Finally, following those conventions means that our assembly functions can be safely called from C or other languages following the System V ABI, without any modification, which is great.
...
> Nasm does have structs, but they are rather a way to define offsets with a name, than structures like in C with a specific syntax to address a specific field. For the sake of simplicty, I'll use the manual way, without `nasm` structs.
...
Then, we do the syscall, check the returned value, exit the program if the value is not 0, and finally return the socket file descriptor, which will be used everytime in the rest of the program when talking to the X11 server.
...
Since an application in X11 can have multiple window, we first need to create a graphical context containing the general information. When we create a window, we refer to this graphical context by id.
...
If you are following along at home, and just ran the program, you have realized nothing is diplayed:
...
Then, we write a small function to read data on the socket. For simplicty, we only read 32 bytes of data, because most messages from X11 are of this size. We also return the first byte which contains the event type.
...
The offical documentation has formulas to compute those values.
...
```x86asm
    ...
    jnz .poll_messages.received_other_event

    .poll_messages.received_exposed_event:
    mov BYTE [rsp + 24], 1 ; Mark as exposed.

    .poll_messages.received_other_event:

    cmp BYTE [rsp + 24], 1 ; exposed?
    jnz .poll_messages.loop

    .poll_messages.draw_text:
...
Pouh, that was a lot. But we did it! We wrote a (albeit simplistic) GUI program in pure assembly, no dependencies, and that's just 600 lines of code at the end.
...
- Stripped and OMAGIC (`--omagic` linker flag) (`Set the text and data sections to be readable and writable.  Also, do not page-align the data segment`): 1776 bytes (1 KiB)
...
global _start:function
  ...
  mov r8d, 200 | (200<<16) ; No x and y specified.
  %define WINDOW_W 800
  %define WINDOW_H 600
