...
> `nasm` uses the intel syntax: <instruction> <destination> <source>, so `mov rdi, 0` puts 0 into the register `rdi`. Other assemblers use the AT&T syntax which swaps the source and destination. My advice: pick one syntax and one assembler and stick to it, both work and most tools have some support for both.
...
**Note**: There is an optimization method that uses `rbp` as a standard register (`-fomit-frame-pointer`), which means we lose the information about the call stack. My advice is: never do this, it is no worth it.
...
> `jle` is a conditional jump, which inspects global flags, hopefull set just before with `cmp` or `test`, and jumps to a label if the condition is true. Here, we compare the returned value with 0, and if it is lower or equal to 0, we jump to the error label. That's how we implement conditionals and loops.
