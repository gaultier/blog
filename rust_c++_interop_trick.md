...
1. For each field of the C++ class/Rust struct, add a compile-time assert to make sure the layout is the same e.g. `static_assert(sizeof(User) == sizeof(UserC)); static_assert(offsetof(User, name) == offsetof(UserC, name));`. If this fails, bail.
