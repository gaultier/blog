...
So in a way, it's not really a *self*-pipe, it's more precisely a pipe between the parent and the child, and nothing gets written to it, it's just used by the child to signal it's done when it closes it. Which is useful for many cases outside of our little program.
