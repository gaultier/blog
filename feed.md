...
- For each article (`*.html`) file in the directory, we add an entry (`<entry>`) in the XML document with:
  ...
  * The 'updated at' field, which is ~~just the `mtime` of the file locally~~ is queried from git
  * The 'published at' field, which is ~~just the `ctime` of the file locally~~ is queried from git
