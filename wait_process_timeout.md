...
- We use `ppoll` instead of `poll`. `ppoll` blocks the `SIGCHLD` signal from interrupting the poll. That's to avoid some data races, quoting from the man page for `pselect` which is analogous to `ppoll`: 
