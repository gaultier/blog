...
- It's easy to accidentally send a signal to all processes with `kill(0, SIGKILL)` or `kill(-1, SIGKILL)` if the developer has not checked that all previous operations succeeded. This is a classic mistake: `child_pid = fork(); ... (do not check that fork succeeded); kill(child_pid, SIGKILL);` which results in `kill(-1, SIGKILL)`
